...
void btnTick() {
  ...
  // if the button was pressed several times before the timeout
...
void swingTick() {
  if (GYR > 80 && (millis() - swing_timeout > 100) && HUMmode) {
    ...
    if (((millis() - swing_timer) > SWING_TIMEOUT) && swing_flag && !strike_flag) {
      ...
      if ((GYR > SWING_L_THR) && (GYR < SWING_THR)) { //300 > GYR > 150
        nowNumber = random(5);
        // read track name from PROGMEM
